...
## D-042 · O código do rodapé precisa de caminho de volta (2026-09-02)
- **Contexto:** o modo debug punha no rodapé da resposta um código curto derivado do `trace_id`, e a documentação prometia que "o cliente manda o código e você acha a linha". Nenhum comando fazia isso: `short_code` só era usado para **escrever** o código, nunca para procurá-lo. A primeira ligação de suporte terminaria em SQL escrito na mão contra `audit_logs`.
- **Escolha:** `orbi trace show <código|trace_id> --tenant <slug>`, que devolve o turno inteiro — quem perguntou, a pergunta literal, a tool escolhida, qual entidade foi resolvida e por qual etapa da cascata, a decisão da policy, a latência por etapa, tokens e custo, e o hash do payload do ERP.
- **Motivo:** meia promessa é pior que promessa nenhuma, porque ela aparece na venda e falha no atendimento. O código sair na resposta só tem valor se existir o caminho de volta.
- **Detalhe:** o código **não é reversível** — é derivação do `trace_id`, não cifra. A busca calcula o código das linhas da janela e compara, e por isso a janela é limitada (`--dias`, 30 por padrão): a auditoria é particionada por mês, e varrer um ano para achar a reclamação de ontem custaria sem servir. A busca corre dentro da sessão do tenant, então a RLS vale — o código de um cliente não encontra turno de outro, e isso é testado.

## D-043 · Duas origens de número, e o cliente escolhe (2026-09-02) — *substituída por D-048*
- **Contexto:** o número de WhatsApp pode viver no Business Portfolio do cliente (ele verifica o CNPJ e libera acesso ao Orbi como Tech Provider) ou no nosso (nós criamos e operamos). O caminho oficial da Meta — Tech Provider Program com Embedded Signup — contempla os dois: número atribuído pelo Tech Provider, ou número do próprio cliente verificado por OTP.
- **Escolha:** oferecer as duas como opções de primeira classe, na venda, e o cliente decide.
- **Motivo:** clientes chegam em estados diferentes. Quem já tem Business Portfolio verificado prefere manter o controle e sai em uma hora; quem não tem enfrentaria dias a semanas de verificação com documento, e essa fricção mata uma PoC de três semanas. Recusar uma das duas é perder cliente por um motivo que não é de produto.
- **O código é indiferente às duas.** O Orbi guarda `channel_address`, `channel_phone_number_id` e o token cifrado; onde a WABA mora é assunto da Meta, não do esquema. Nenhuma linha muda entre os dois cenários — o que confirma que esta é decisão comercial e operacional, não técnica.
- **O risco que o cliente não vê, e que é nosso administrar:** portfólio da Meta desabilitado por violação de integridade trava **todas** as WABAs dentro dele. Hospedar dez clientes num único portfólio significa que a violação de um derruba os dez. Mitigação para os números que hospedarmos: portfólio separado por cliente hospedado, para que o raio de explosão continue sendo um. Enquanto forem poucos isso é gerenciável; quando deixar de ser, é sinal de que a opção hospedada precisa de preço próprio.
- **Não muda:** um número por cliente. `tenants.channel_address` é único, e a busca passa pela função `orbi_tenant_by_channel` — que já é o ponto de extensão caso um cliente peça vários números **com significado** (o número da filial que lidera pelo estoque dela). Sem o significado, dois números são um número com custo dobrado.

## D-044 · A confiança do alias precisa mudar o comportamento (2026-09-09)
- **Contexto:** o D-013 prometia que "um toque errado não pode envenenar a resolução do cliente para sempre", e a coluna `entity_aliases.confidence` existia para isso. Auditando: ela era **escrita** em quatro lugares e **lida em um** — para imprimir numa tabela da CLI. A cascata de resolução nunca a consultava.
- **A falha real:** um único toque errado numa desambiguação criava um alias `low` que resolvia no **primeiro** estágio da cascata, com `score=1.0`, sem passar por limiar nenhum. A próxima pessoa da mesma empresa a usar aquele termo recebia o saldo do produto errado, com confiança e sem ambiguidade. Pior: cada uso incrementava `hits` — onde "uso bem-sucedido" significava apenas "o alias foi usado", não "a resposta estava certa" — então o alias errado se **promovia a `confirmed`** justamente por continuar sendo usado. O mecanismo desenhado para impedir o envenenamento acabava ratificando-o.
- **Escolha:** `_by_alias` passa a devolver a confiança, e `_conciliar` reconcilia um alias `low` com o catálogo. `confirmed` resolve direto. `low` resolve quando o catálogo concorda ou quando o catálogo não acha nada (a gíria que só aquele cliente usa — a razão de o alias existir). Quando o catálogo aponta outra entidade, **pergunta**.
- **Motivo:** é a regra do produto aplicada onde ela mais vale. Discordância entre o que uma pessoa tocou uma vez e o que o catálogo diz é dúvida, e dúvida pergunta. Este era o caminho mais direto para o erro silencioso — responder com confiança a entidade errada — que toda a calibração existe para manter abaixo de 1%.
- **Custo:** um alias `low` passa a gastar a consulta por trigrama que antes pulava. Dura até a promoção, que são dois usos.
- **Como isso passou:** havia testes de alias, e eles verificavam a **coluna** mudando de `low` para `confirmed`. Nenhum verificava que o valor da coluna mudava alguma coisa. O teste novo falha no comportamento anterior e passa no atual — foi verificado nos dois sentidos.

## D-045 · Ir fundo em um ramo antes de atender vários (2026-09-16)
- **Contexto:** a pergunta era se o Orbi deve servir a sistemas além de ERP — empréstimo, rastreamento, seguros — para não ficar preso a um mercado só. Medindo o código: **cerca de 12% está preso ao domínio** (tools, DTOs, adapter Odoo, lista de campos, templates, datasets de eval); os outros ~88% — canal, identidade, Policy Layer, papéis, resolução, auditoria, LLM, deadline, RLS, rate limit, observabilidade — servem a qualquer sistema de registro. E as menções a ERP no núcleo (`erp_entity_id`, `erp_supported_tools`, `erp_payload_hash`) são **nome**, não acoplamento: nenhum módulo central ramifica em semântica de ERP.
- **Escolha:** não generalizar o produto agora. Ir fundo em **distribuidor e atacado** até o terceiro cliente pagante; a primeira expansão é **rastreamento e logística**, vendida ao cliente que já existe.
- **Motivo:** o código é a parte barata. O que custa num domínio novo é saber quais são as quatro perguntas certas, o adapter de referência com Conformance Kit, os evals, o vocabulário acumulado e um cliente que dê credibilidade — 12% do código, praticamente 100% do conhecimento de mercado. A razão que decide: segundo cliente do mesmo ramo sobe em **horas**; segundo cliente com outro ERP, em **dias** (um adapter); primeiro cliente de um ramo novo, em **semanas e sem referência nenhuma**. Enquanto a razão à esquerda não estiver provada com dinheiro, mudar de direção é caro.
- **O que isso preserva:** a opcionalidade já está guardada na arquitetura — `ErpAdapter` é Protocol, o Tool Registry declara cada tool uma vez, a Field Policy é indexada por permissão e não por domínio. Renomear `erp_*` para algo neutro custa uma tarde, no dia em que houver motivo. O erro seria **gastar** essa opcionalidade antes do primeiro cliente.
- **O filtro para qualquer ramo novo** está em [ORBI-COMERCIAL.md](ORBI-COMERCIAL.md): seis condições, todas derivadas da arquitetura, e um ramo só entra quando cumpre as seis.
- **Seguros ficam de fora por razão técnica, não de mercado:** cobertura de apólice é interpretação de contrato, não campo de banco. Responder isso exigiria o LLM redigir a resposta — a única linha que o produto não cruza (D-001). Atender seguros exigiria desmontar o que o diferencia.

## D-046 · O CI instala o que a suíte exercita, e roda por entrega (2026-09-17)
- **Contexto:** o CI estava vermelho em **29 dos últimos 30 runs**, em `main` e em `development`, desde 25/08 — enquanto a suíte passava localmente e era reportada como verde. Causa: o CI instalava `.[dev]`, e o extra `dev` não incluía os SDKs do Gemini (`google-genai`) nem do Langfuse; onze testes caíam com `No module named google`. Na máquina de desenvolvimento os SDKs estavam instalados por outro motivo, e o `gh` não estava disponível para ler o resultado remoto. Resultado: semanas de e-mail de falha, e uma afirmação ("CI verde") que não era verificada onde importa.
- **Escolha 1:** o extra `dev` passa a incluir `orbi[llm,observability]`. `pip install -e ".[dev]"` seguido de `pytest` tem que passar — para o CI e para quem chega ao projeto pelo README. Um teste de arquitetura nomeia o extra ausente em uma linha em vez de onze falhas.
- **Escolha 2:** o CI roda em `push` para `main`, `pull_request` para `main` e `workflow_dispatch`. Não roda em push para `development`. A verificação é por entrega; iterar em `development` não gera runs nem e-mails. Cada merge para `main` produz **um** run, e um run verde não manda e-mail.
- **Consequência honesta:** com o gatilho só em `main`, a verificação acontece **depois** do merge, não antes — o princípio "regressão de eval bloqueia merge" (ORBI.md §14) passa a valer só via PR. Enquanto o fluxo for merge direto, o que bloqueia é o hábito de conferir o run. Se um dia houver PR, o gatilho de `pull_request` já está lá.
- **O que muda no meu procedimento:** depois de todo push para `main`, ler o resultado do run pela API pública do GitHub (o repositório é público) antes de afirmar que está verde.

## D-047 · Com cliente pagante, OpenAI primário e Anthropic fallback; Gemini só em dev e PoC (2026-09-17)
- **Contexto:** os documentos deixavam a escolha do fabricante para depois ("ative billing no Google **ou** vá para OpenAI, meça antes"). A decisão do dono do produto fecha isso. Além disso, o código carregava defaults que já contradiziam o D-035: `gemini-3.7-flash` com thinking `0`, onde a decisão era `gemini-3.5-flash-lite` com `-1` — funcionava só porque o `.env` sobrescrevia.
- **Escolha:** cliente pagante roda em OpenAI (`gpt-5-mini` como default) com Anthropic (`claude-sonnet-5`) de fallback — dois fabricantes pagos, como a regra do failover exige. Gemini fica para desenvolvimento e PoC, na camada gratuita. Os defaults do código passam a ser exatamente os das decisões.
- **Motivo:** a camada gratuita do Gemini tem cota de 20 perguntas/dia por modelo e variância que passou de 20 s numa chamada; nenhuma das duas serve a um cliente com equipe. O custo não decide: R$ 6–30 por cliente/mês em qualquer fabricante desta lista. O que ainda se mede é o **modelo dentro do fabricante** (`gpt-5-mini` vs `gpt-4.1-mini`), com o `orbi bench`, no dia em que houver chave.
- **Default `gpt-5-mini` em vez de `gpt-4.1`:** o `4.1` custa 8× mais na entrada sem nenhuma medição a favor; o `5-mini` é da geração atual e é o modelo usado nos exemplos comerciais. É default, não veredito — uma linha no `.env` troca.
- **O que isso não muda:** um modelo para todos os clientes (D-036) — o fabricante mudou, a regra não.

## D-048 · O número é nosso por padrão; o do cliente, só se ele insistir (2026-09-17)
- **Contexto:** o D-043 oferecia as duas origens com o cliente escolhendo e o número dele como padrão. A decisão do dono do produto inverte: **criamos o número na nossa conta da Meta**, para ter controle da operação e subir no dia, sem depender da verificação de CNPJ do cliente nem de ele nos dar acesso. O número do cliente entra só se ele insistir — e aí vale, porque o código é indiferente (endereço, `phone_number_id`, token cifrado).
- **O que não muda:** um Business Portfolio **por cliente hospedado**, sem exceção — portfólio desabilitado por violação de integridade trava todas as WABAs dentro dele.
- **O que fica mais importante:** o teto de portfólios da Meta. Com hospedar como padrão, ele deixa de ser um caso raro e vira o limite do próprio padrão: fontes públicas divergem entre 2 e 5 portfólios por pessoa, sem confirmação oficial. Conferir o limite no Business Manager **antes do segundo cliente** e pedir aumento; se a Meta não conceder, a partir do teto os próximos clientes usam o número deles — e isso precisa estar dito na venda, não descoberto na implantação.
- **Na saída do cliente:** migrar o número para o portfólio dele ou ele troca de número. É um passo a mais que o D-043 não tinha; está no roteiro de encerramento como item explícito.
